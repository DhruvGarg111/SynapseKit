...
- **Visual-document multimodal RAG** (#894) — page-level retrieval and answering over scanned PDFs and slide decks, additive alongside the existing text-only `RAG` API. Provider-agnostic visual-page contracts and lazy `ColPaliEmbeddings`/`ColQwenEmbeddings` (Transformers-backed, no eager import) power late-interaction MaxSim retrieval over rendered pages (`VisualDocumentRetriever`); `RAG(visual_embeddings=..., visual_renderer=...)` (or the named `MultimodalRAG` alias) indexes PDFs/PPTX/images alongside OCR/text fallback, and `ask`/`stream` attach page images with deterministic page/bbox/source citations for providers that declare `supports_multimodal = True` (Anthropic, OpenAI, Azure OpenAI, Gemini), degrading to text-only prompting otherwise. New `visual` extra; offline benchmark + `visual-rag-gate` CI job. Post-merge review fixed seven bugs before merge: `ImageLoader` silently degraded to placeholder captioning for the ~55 LLM providers that don't yet declare `supports_multimodal` (now logs a warning instead of failing silently); MaxSim scoring ran synchronously on the event loop, violating the async-first invariant (now offloaded via `asyncio.to_thread`); `_filter_candidates` swallowed unhashable metadata-filter values (e.g. a `bbox` list) and silently returned no results instead of raising; visual-vs-text answer routing was decided purely by whether *any* page had ever been indexed rather than by per-query retrieval results (now falls back to the text pipeline when the visual retriever returns nothing); visual-page rendering failures during ingestion (corrupted/password-protected files) were swallowed by a bare `except` with no logging; `ColPaliEmbeddings._prepare_image` passed an empty base64 image through to the model instead of raising, crashing opaquely deep in transformers/PIL; and degenerate zero-norm token vectors were silently left unnormalized in embedding validation instead of being logged. New tests also replaced `MagicMock`/`AsyncMock`/`unittest.mock.patch` usage with hand-written fakes and `pytest`'s `monkeypatch`, per house testing standards. Contributed by [@DhruvGarg111](https://github.com/DhruvGarg111).

- **Event-driven streaming ingestion** (#891) — an async-first `StreamingIngestor` pipeline (normalization, bounded batching, backpressure, acknowledgements, retries, graceful shutdown) that keeps a `VectorStore`, `WorldModelRAG`, or `KnowledgeMesh` current as events arrive. Optional sources for Kafka/Redpanda, Pulsar, AWS Kinesis, Debezium-style CDC, and PostgreSQL logical replication, each lazily imported behind its own extra. Durable SQLite/in-memory checkpoints plus idempotent upsert targets give at-least-once delivery with exactly-once-ish effects; event timestamps are preserved as `valid_at` so `WorldModelRAG` can append distinct temporal graph edges as CDC rows change over time. Includes a real Kafka Testcontainers acceptance test wired into the `integration` CI job. Post-merge review fixed six correctness bugs: `StreamingIngestor.stop()` could tear the source down mid-batch and silently drop already-persisted Kafka acks (now serialized behind a batch lock); the delete/tombstone path only worked for Debezium CDC records because `document_id` was never set for Kafka/Kinesis/Pulsar/JSON sources (now derived from the source message key, the standard log-compacted-topic identity convention); `WorldModelRAG.ingest`'s new timestamp backfill collided with an unrelated pre-existing `metadata["timestamp"]` key already used by the Kafka/Slack/Teams loaders (now scoped to `valid_at` only); LLM-extracted `valid_at` could vary between re-extractions of the same replayed event and silently fork the graph edge id (the document's own deterministic timestamp now takes precedence); `valid_until` could never be corrected once set, which streaming turns into a routine occurrence for CDC row updates (a newly supplied bound now wins); and `KnowledgeMeshSink.upsert` could leave the on-disk mesh index stale if a step after `rag.ingest` failed (the index is now persisted regardless). Contributed by [@DhruvGarg111](https://github.com/DhruvGarg111).
- **Multi-agent orchestration eval (loops, context loss, mis-routing)** (#983) — goal-level, orchestration-aware evaluation over a framework-agnostic `RunGraph` abstraction (`RunNode`, `Transfer`) detecting three core failure modes: infinite handoff loops (`LoopDetector` with exact periodic cycle detection + stalled loop input similarity checks), per-transfer context loss (`ContextLossDetector` comparing extracted factual spans/IDs across handoffs, plus an opt-in async `LLMContextLossJudge`), and non-deterministic mis-routing (`MisroutingDetector` computing target agent modal distribution across goals or checking against a golden route). `OrchestrationEvaluator` aggregates findings into an `OrchestrationEvalReport` (JSON and Markdown output). **Wired into SynapseKit Live** — `evaluate_and_publish` emits `orchestration.eval` events and live graph snapshots with annotated edge findings, rendered in the Live dashboard (🧭 icon + story line). Zero new hard dependencies, fully typed, backed by deterministic structural fixtures (looping, context-loss, mis-routing, and a false-positive-free healthy fixture), a runnable `examples/orchestration_eval.py`, and an `orchestration-eval-gate` CI job. Part of Epic #947.
- **Guardrails & policy middleware** (#892) — a unified, auditable I/O governance layer that runs pre-call (input) and post-call (output) checks on any LLM with four enforcement modes: `block`, `redact`, `flag`, and `require_human`. Ships input guards (`PromptInjectionGuard`, `JailbreakGuard`, `TopicGuard` allow/deny, `MaxCostGuard`) and output guards (`PIIRedactionGuard` reusing the battle-tested `MemoryPIIFilter`, `ToxicityGuard`, `CitationRequiredGuard`, and a `SchemaGuard` that validates output against a Pydantic model with no extra model call). A `GuardrailPolicy` composes guards over both stages, rolls their findings into a single report whose action is the most-severe triggered mode (redact-mode hits chain, rewriting the text seen by later guards), and `GuardedLLM` wraps any `BaseLLM` so the policy applies transparently on `generate`/`stream`/`*_with_messages` — a `block`/`require_human` guard raises `GuardrailBlockedError` before the model is called, output redaction rewrites the response, and input redaction propagates into the wrapped call. Three ready-made **compliance rulepacks** (HIPAA/GDPR/PCI-DSS) map each regime's data-minimisation intent onto concrete guards. **Every decision emits a signed, replayable audit record** (reusing the `audit` hash-chain + Ed25519 signing, verdict tagged with a self-reported `GroundedSignal`) and a `guardrail.*` SynapseKit Live event — both carry only guard names, counts, and the chosen action, **never the inspected text or any secret**. `GuardrailPolicy.export_audit_bundle()` writes an independently-verifiable bundle. Pure-Python, zero new dependencies, importable without extras; wired into the Live dashboard (🛡️/🛑 story lines). Includes an adversarial test suite (injection/jailbreak blocked, zero false positives on a benign set), async-contract asserts, and an offline `examples/guardrails_hipaa.py`.
- **CAG/RAG router with a llama.cpp KV-cache backend** (#1000) — a provider-independent `CAGRouter` that wraps a standard `Retriever` and, for a stable corpus that fits the context window, uses Cache-Augmented Generation (a precomputed, persisted llama.cpp KV-cache state) instead of retrieval, transparently falling back to RAG when the backend is unsupported, the corpus is unstable/too large, or the cache misses. Ships a pickle-free `KVCacheStore` (atomic `.bin`/`.json` writes, format-version gated) and a `LlamaCppCAGBackend` (`build_cache`/`generate_with_cache`/`load_state`) behind the `llamacpp` extra; other providers route to RAG. Exported from `synapsekit.rag`. Post-merge review fixed three bugs: the corpus fingerprint hashed `sorted(texts)` while the KV cache is built in insertion order (reordered corpora collided on one key with mismatched states — now order-sensitive); the cache key used the `config.model` label rather than the model file, so different GGUFs/quantizations could collide and load a foreign KV blob into llama.cpp (now keyed on the model path + size via `model_cache_id`); and `generate_with_cache` buffered a whole generation on an unbounded queue and leaked its producer thread on an early break (now bounded + stops on `aclose`). Mocked tests were replaced with hand-written fakes and the real `KVCacheStore`. Known limitation: the router primes the KV state but reuse by downstream generation requires the prompt's token prefix to match the raw corpus (a templated pipeline can defeat it) — tracked as a follow-up. Contributed by [@Premvkmishra](https://github.com/Premvkmishra).
- **Sandboxed PC Twin** (#747) — a local-first, auditable way to let an agent work against a copy-on-write clone of a chosen host root, review the exact filesystem changes, gate them behind an explicit evaluation, and apply them back transactionally. `PCSandbox` materializes a deterministic overlay (secret-bearing dotfiles like `.env`/`.ssh`/`.aws`/`.netrc` excluded by default, symlinks never followed outside the base), runs work in a hardened backend (Docker/OrbStack with `--read-only`, `--cap-drop ALL`, `no-new-privileges`, `--network none` default, pids/memory/cpu caps; Lima and Firecracker fail closed when unavailable; a `fake` backend for tests only), and produces a reviewable, digest-verified `DiffBundle`. Applying a bundle requires a passing `EvalReceipt` bound to that exact diff digest and is fully transactional — pre-apply `before_sha256` conflict detection, a backup journal, post-apply `after_sha256` verification, and ordered rollback on any failure. All host paths are normalized (absolute/`..`/NUL/symlink-parent rejected) so a bundle can never write outside the target root. New `synapsekit sandbox spawn|diff|apply|discard` CLI, and a `ComputerUseAgent(env=…)` integration whose audit events record action/observation **metadata only** (`text_length`, never the typed text or screen contents). All blocking filesystem and subprocess work is offloaded off the event loop. At merge, added negative-security regression tests locking the traversal/duplicate-path/digest-tamper/symlink-escape rejections and the no-keystroke-logging audit property; two follow-ups filed for a bounded-output cap on sandbox exec (#1016) and optional signed-receipt verification at apply time (#1017). Contributed by [@DhruvGarg111](https://github.com/DhruvGarg111).
- **11 new LLM providers** (#887) — eight OpenAI-compatible adapters (DeepInfra, Nebius AI Studio, Baseten, Upstage Solar, Reka, Hyperbolic, FriendliAI, Clarifai) on lazy `AsyncOpenAI` clients, plus three bespoke native-async REST integrations: **NVIDIA NIM** (hosted `integrate.api.nvidia.com` and self-hosted/unauthenticated deployments), **IBM watsonx.ai** (IAM API-key → bearer-token exchange, project/space scoping, `tool_choice_option` tool calling), and **Snowflake Cortex** (PAT/JWT bearer + `X-Snowflake-Authorization-Token-Type`). All register in the LLM `_factory` and top-level lazy exports, each behind its own extra (`nvidia-nim`, `watsonx`, `snowflake-cortex`, `deepinfra`, `nebius`, `baseten`, `upstage`, `reka`, `hyperbolic`, `friendli`, `clarifai`), with streaming + native tool calls and `respx` HTTP-contract tests (opt-in live smoke tests skipped without credentials). Also adds `BaseLLM.aclose()` / async-context-manager support so lazily-created provider clients release their connection pools, cumulative token accounting, and `top_p` forwarding across every provider. New providers subclass `BaseLLM`, so existing SynapseKit Live tracing, caching, retries, and rate limiting apply unchanged. Post-merge review fixed: watsonx streaming POSTed to the non-streaming `/ml/v1/text/chat` endpoint with a schema-invalid `stream` body field — real watsonx.ai streams only from `/ml/v1/text/chat_stream`, and the `respx` mock hid it by serving SSE from the wrong URL (#942); the eight OpenAI-compatible providers silently dropped a configured `config.timeout` instead of forwarding it to `AsyncOpenAI`; the watsonx IAM token was cached until its exact expiry instant (could 401 on a token that lapsed in flight — now refreshed 60s early); and the watsonx/Cortex/NIM tool-call parsers ran `json.loads` on provider arguments unguarded (now degrade to `{}` on a malformed payload). Contributed by [@DhruvGarg111](https://github.com/DhruvGarg111).
- **9 new vector stores** (#888) — async-first adapters for **Turbopuffer**, **Azure AI Search**, **Vertex AI Vector Search**, **SingleStore**, **TiDB Vector**, **Couchbase**, **SurrealDB**, **Deep Lake**, and **MyScale**, each behind its own extra (`turbopuffer`, `azure-ai-search`, `vertex-vector`, `singlestore`, `tidb`, `couchbase`, `surrealdb`, `deeplake`, `myscale`) and lazily imported. All implement the full `VectorStore` contract (`add`/`search`/`search_mmr`/`save`/`load`) via a shared `RemoteVectorStoreSupport` mixin that handles document normalization, metadata round-trip, MMR over provider candidates, a portable JSON snapshot for `save`/`load`, and sync-client offloading through `asyncio.to_thread` (async-blocking gate clean). SQL-backed adapters parameterize the query, validate identifiers, and cast `top_k`; Vertex persists a local datapoint→document map because its nearest-neighbour response returns IDs only. Bumps the vector-store count to 31. Post-merge review fixed: `CouchbaseVectorStore.search` returned empty `text`/`metadata` because it never requested `SearchOptions(fields=["*"])` — the Couchbase FTS SDK returns only id+score by default, and the smoke-test fake hid it (#1013). Follow-ups filed for Turbopuffer score-vs-distance consistency (#1014) and Deep Lake reconnect via the version fallback path (#1015). Contributed by [@DhruvGarg111](https://github.com/DhruvGarg111).
...
- **Dream Mode — local-first overnight self-reflection** (#749) — an explicit, opt-in runner that, while the machine is idle and plugged in, replays the day's **signed** audit traces, distills durable lessons, proposes (never auto-applies) `LivingMemory` patches, refreshes `KnowledgeMesh` state, flags stale memories (report-only, never deletes), and emits a signed audit bundle plus a concise morning briefing. Everything is gated: constructing `DreamMode` starts no background task, memory patches stay pending for human review, entity consolidation is report-only, and the scheduler fails closed when power state is unknown or the machine is unplugged. Token/transcript limits bound overnight work; an injectable model backend (e.g. `EdgeRuntime`) keeps inference local. New `synapsekit dream run|status` CLI. Trace bundles are validated with `ReplayEngine`/hash-chain verification before distillation — records are relinked into true chain order first, so runs whose events share a timestamp are no longer silently dropped. `ingest_traces` is async (offloads its SQLite writes) and `dream status` is fully read-only. **Wired into SynapseKit Live** — every `run_once` publishes a `dream.run` event (outcome, trace/lesson/patch counts, or the skip reason) so overnight reflection shows in the glass-box dashboard (#934). **Audit bundles are now attestable** (#935) — DreamMode persists a per-install Ed25519 signing key (`0600`, at `<state dir>/signing_key` or `DreamConfig.signing_key_path`) so every night's bundle is signed by the *same*, pinnable key; `DreamMode.trusted_keys()` feeds `verify(bundle, trusted_keys=…)` for a real non-repudiable `MATCH` (unpinned `verify()` stays `UNVERIFIABLE` by design), and the morning briefing surfaces the key id or a non-attestable warning. Contributed by [@DhruvGarg111](https://github.com/DhruvGarg111).
- **Ambient daemon (MVP)** (#743) — an opt-in background daemon that watches a git repo for dirty state and the terminal for risky commands (destructive delete, `git reset --hard`, force-push/clean), firing a Windows toast when a risky command lands against uncommitted work. Rule-based (no ML), with a `MeshDaemon`-style start/stop/status lifecycle, an `AuditLog` trail, and a per-source privacy opt-out (`~/.synapsekit/ambient.ignore`). Command text is **redacted** (via the mesh secret filter) before it enters the audit log, the daemon prints an explicit shell-history disclosure notice at startup, and all toast/audit/status IO is offloaded off the event loop (async-first). `stop()`/`status()` reconcile against process liveness so a surviving or `kill -9`'d daemon is never misreported. New `synapsekit ambient start|stop|status|log` CLI and `ambient` extra (`plyer`). **Wired into SynapseKit Live** — each fired intervention publishes an `ambient.intervene` event (rule, confidence, source — never the raw command text) so ambient activity shows in the glass-box dashboard (#934). MVP scope — `editor`/`browser` sources, an ML intent classifier, and macOS/Linux notifiers are follow-ups. Contributed by [@Abhay-Mmmm](https://github.com/Abhay-Mmmm).
- **14 new data loaders** (#889) — async-first loaders for **Asana**, **Box**, **ClickUp**, **Databricks**, **Delta Lake**, **Figma**, **Gmail**, a generic **GraphQL** endpoint, **Apache Iceberg**, **Kafka**, **Linear**, **Monday.com**, **Shopify**, and **Zoom**, each behind its own extra and lazily imported. The HTTP loaders share `_http_utils`/`_record_utils` helpers (labelled record→`Document` text, metadata round-trip, and cursor/offset/`Link`-header pagination — no silent first-page-only reads); SDK-backed loaders (Databricks/Delta/Iceberg/Kafka/Gmail) keep blocking driver calls off the event loop via `run_in_executor`, and Kafka ships a real testcontainers integration test wired into CI. All outbound HTTP is SSRF-guarded (see Security, #1029). Bumps the loader count to 84. Contributed by [@DhruvGarg111](https://github.com/DhruvGarg111).
- **Cloud persistence backends for agent memory & graph checkpoints** (#890) — six new `AgentMemory` backends (**MongoDB**, **DynamoDB**, **Cassandra**, **ScyllaDB**, **Azure Cosmos DB**, **Google Firestore**) and matching graph `Checkpointer`s, each behind its own extra and lazily imported. A shared `_serialization` helper round-trips embeddings (big-endian `struct` doubles + dimension), JSON metadata, and tz-aware ISO-8601 timestamps identically across every store; identifiers are validated with the house allow-list and all record values are parameterized (DynamoDB `Binary`/`Decimal`, Cosmos parameterized queries, Cassandra prepared statements). Sync drivers (pymongo, boto3, cassandra-driver, sync azure-cosmos) are offloaded via `asyncio.to_thread`; Firestore/Cosmos use their native async clients — async-blocking gate clean. Real testcontainers integration tests (Mongo/Cassandra/DynamoDB/Firestore) wired into CI. Contributed by [@DhruvGarg111](https://github.com/DhruvGarg111).
- **Code Archaeology Agent** (#744) — reconstructs *why* code exists in its current form by fusing git history, local markdown notes, and (opt-in) Slack/email archives. `TimelineReconstructor` merges multi-source events chronologically; `EvolutionDiff` traces a file/symbol's version-level rationale from commit context; `CausalLinker` extracts LLM-identified cause → effect claims from the timeline, with optional `NeuroSymbolicAgent`-backed plausibility checks (documented as a best-effort signal, not a formal proof — free-text causal claims aren't the decidable constraint problems the solver is built for); `ArchaeologyAgent.explain(query)` ties it together into a cited markdown narrative. Reuses the existing `timetravel` (`GitBackend`/`EvolutionIndex`), `loaders` (`SlackLoader`/`EmailLoader`), and `symbolic` modules — zero new hard dependencies. Post-merge review fixed: email timestamps parsed as ISO instead of the real RFC 2822 `Date` header (silently corrupted every email-sourced event to "now"); `EvolutionDiff.trace()` silently falling back to the *entire* unfiltered repo history on an unmatched natural-language query (the common path via `explain()`); a duplicate full git-history walk per `explain()` call (`TimelineReconstructor`/`EvolutionDiff` now share one `EvolutionIndex`); a silently-swallowed LLM construction failure; sequential (now concurrent) causal-claim verification; and 5x-duplicated query-term-extraction logic consolidated into one helper. Contributed by [@DhruvGarg111](https://github.com/DhruvGarg111).
...
- **Hive withdrawal is now reflected in `transparency()`** (#931) — `SQLiteHiveStore.revoke()` updated the `revoked` column, but `list()` rebuilt each envelope from the stored JSON blob (still `revoked: false`) and ignored the column, so `transparency()` reported `withdrawn=false` even after a real withdrawal. `list()` now overlays the `revoked` column onto the reconstructed envelope. Surfaced by a new real urllib-transport → live-service HTTP round-trip test.
- **`FirestoreMemoryBackend` no longer uses the deprecated positional `.where()` form** (#1032) — `fetch`/`clear`/`count` queried with `collection.where("agent_id", "==", agent_id)`, deprecated in `google-cloud-firestore` in favor of `.where(filter=FieldFilter(...))`; a future SDK major removes the positional form entirely, which would have raised instead of warned. Regression test runs against the real Firestore emulator with warnings escalated to errors, confirmed to fail on the old code.
- **`TurbopufferVectorStore.search` now returns a cosine similarity instead of raw distance** (#1014) — every other SynapseKit store returns a higher-is-better similarity, but Turbopuffer's raw `dist` is lower-is-better; a `score > threshold` filter or a cross-store score comparison silently inverted. `_search_sync` now converts via `1 - dist` when `distance_metric="cosine_distance"` (the default); result ordering was always correct, so non-cosine metrics (e.g. `euclidean_squared`, which has no 1:1 similarity mapping) are left as a raw distance. Regression test covers both metrics, confirmed to fail on the old code.
- **`PCSandbox` now plumbs `max_output_bytes` for parity with `command_timeout`** (#1035) — `PCSandbox.__init__` accepted `command_timeout` but not `max_output_bytes`, so every sandbox silently built its `SandboxConfig` with the 1 MB default; `attach()` serialized `max_output_bytes` to `metadata.json` but never read it back, so a round-tripped custom value would have silently reset. Both are fixed (with a backward-compatible default for metadata written before this fix), plus a regression test that reattaches a persisted session and confirms the custom value survives — confirmed to fail on the old code.
- **`BoxLoader` no longer mangles binary file content** (#1030) — `load()` unconditionally did `content.decode("utf-8", errors="replace")` on every downloaded file, turning PDFs/images/other binaries into mojibake. Non-UTF-8-decodable content now gets a `[Binary file: <content-type or extension>]` placeholder with the raw bytes preserved in `metadata["raw_content"]` (plus `metadata["content_type"]` from the response), matching `S3Loader`'s pattern. Regression test confirmed to fail on the old code.
- **`DeepLakeVectorStore`'s in-process fallback no longer returns nothing after a reconnect** (#1015) — when the native `dataset.search(...)` signature is unavailable (SDK version mismatch), the store falls back to an in-process cosine-similarity scan over `_documents`/`_local_vectors`; those caches are only ever populated by `add()` calls made in the current process, so a freshly-constructed instance attached to an already-populated dataset (a reconnect) silently returned `[]` instead of the persisted data. The fallback path now hydrates the local cache from the dataset's persisted rows (when the dataset supports `len()`/row-style indexing) before scanning. Regression test confirmed to fail on the old code; flagged in the issue as version-dependent and worth re-validating against a real Deep Lake dataset.
...
- **Sandbox `run_process` no longer buffers unbounded subprocess output** (#1016) — the sandbox backend base ran `process.communicate()` with no byte cap (the same class as #929 in the shell executor), so a command that writes without bound could exhaust memory. Output is now streamed off both pipes concurrently under a `max_output_bytes` cap (default 1 MB, validated `> 0`) that **kills the child** on overflow, decodes with `errors="replace"`, and appends `[output truncated]`. The cap is a new defaulted `SandboxConfig.max_output_bytes` threaded through `SandboxEnvironment` into every backend's `run_process` (mirroring `command_timeout`); non-breaking (defaulted kwarg). Regression test drives a real overflowing subprocess. Follow-up #1035 tracks plumbing the field through the `PCSandbox` constructor/`attach`. Contributed by [@MohamedIdhries](https://github.com/MohamedIdhries).
...
- **CI now actually verifies Python 3.14 compatibility** — `pyproject.toml` classified SynapseKit as `Programming Language :: Python :: 3.14`-compatible, but the `test` matrix in `ci.yml` only ran 3.11–3.13, so the claim was never checked. Verified locally first (full suite: 5070 passed, 97 skipped on CPython 3.14.3, identical to 3.11–3.13) before adding `"3.14"` to the CI matrix so it's enforced going forward.
- **Cosmos memory/checkpointer integration test can no longer silently pass by skipping** (#1033) — the `cosmos_config` fixture scraped the emulator's startup master key from a single `container.get_logs()` call right after start, with no readiness wait; if the (slow, 1-2 minute) emulator wasn't ready yet, it hit `pytest.skip(...)`, making this the one real-backend fixture in `test_issue_890_integration.py` that could pass-by-skip in CI without Docker being unavailable. A first attempt to fix this by waiting on a "Master Key:" log line surfaced a deeper, real bug: the `vnext-preview` emulator image never logs a master key at all and has no reliable readiness signal (a known upstream limitation, [Azure/azure-cosmos-db-emulator-docker#154](https://github.com/Azure/azure-cosmos-db-emulator-docker/issues/154)) — its API port accepts TCP connections before the backing schema is actually initialized. The fixture now uses the emulator's fixed, publicly documented master key directly and polls with a real `CosmosClient.get_database_account()` call in a retry loop (mirroring the Cassandra fixture's real-connection retry), raising `RuntimeError` instead of skipping if the backend is never actually ready. That real-connection attempt then surfaced a *third*, pre-existing bug that had never been reachable before (the fixture always skipped first): it built the endpoint URL from the container's exposed port `1234` instead of `8081`, the actual Gateway/API port for this image; `1234` is an unrelated secondary port and every connection to it failed. Fixing that surfaced a *fourth* pre-existing gap: `azure.cosmos.aio.CosmosClient` (used by both the real backend and the fixture's readiness probe) needs `aiohttp` as its async transport, which was never in the `integration` dependency group. Added `aiohttp>=3.14.3`. Also moved the Cosmos test into its own `cosmos-integration` CI job (mirroring the existing `ollama-integration` isolation) so a slow emulator can't destabilize the main `integration` job.

### Security

- **Triaged 3 `chromadb` advisories — SynapseKit users are not affected; no bump possible.** A fresh OSV audit of the full resolved graph (538 packages) flagged only `chromadb` (GHSA-36p7-vc44-83pf / CVE-2026-45833 critical code-injection, GHSA-2wm9-hf6c-p5cr cross-tenant read/write, GHSA-xph7-9rjv-w5fr RBAC scoping). None has a patched release — the latest PyPI version (1.5.9) is the top of every vulnerable range — so there is nothing to bump to. All three require running the **chromadb server** with authentication/multi-tenancy enabled; per each advisory the embedded clients (`PersistentClient`/`EphemeralClient`) do not expose the vulnerable endpoints, and SynapseKit only ever uses chromadb embedded (`retrieval/chroma.py`). Documented the assessment on the `chroma` extra pin (already capped `<1.0.0`) and dismissed the Dependabot alerts as "vulnerable code not used." No dependency or public API changed.
...
- **Fixed a path-confinement bypass in `FileReadTool` / `FileWriteTool`** (#1021) — the `base_dir` sandbox used a raw `str(resolved).startswith(str(base_dir))` check, which a sibling directory sharing a string prefix (`/ws` vs `/ws-secret`) slipped past, allowing an LLM-supplied path to read — and, via `FileWriteTool`'s `mkdir(parents=True)`, create/overwrite — files outside the sandbox. Now uses `Path.is_relative_to`, matching the correct precedent in `mesh/loaders.py` and `sandbox/apply.py`. As defense-in-depth, `FileListTool` and `PDFReaderTool` gain the same optional `base_dir` confinement. Non-breaking: legitimate in-tree paths are unaffected.
- **Closed SSRF holes in the agent HTTP tools and RSS loader** (#1022) — `HTTPRequestTool`, `GraphQLTool`, and `APIBuilderTool` fetched LLM/spec-derived URLs with no SSRF guard (reaching `169.254.169.254` cloud metadata, `127.0.0.1`, RFC1918 services, or `file://`), `WebScraperTool` rolled its own guard that failed **open** on DNS errors and inspected only the first IPv4 address, and `RSSLoader` passed unvalidated URLs (incl. `file://`) to `feedparser`. All now route through the shared fail-closed guard in `loaders/_url_guard.py` (scheme allowlist, resolves and rejects every private/loopback/link-local address, IPv4-mapped-IPv6 normalized). aiohttp/urllib auto-redirects are followed manually so each hop is re-validated; `RSSLoader` still accepts bare local paths for backward compatibility but rejects `file://` and other non-http(s) schemes. Regression tests hit each site with metadata/loopback/RFC1918/`file://` targets (IP literals — no network).
- **Bumped 3 optional-extra dependencies off known-CVE versions** (#1025) — an OSV audit of the full resolved graph (549 packages, all extras + groups) flagged three fixable advisories. Floored `transformers>=5.10.0` (CVE-2026-9856, `save_pretrained` chat-template path traversal → arbitrary file write; re-locked 5.9.0 → 5.16.1), `snowflake-connector-python>=4.7.1` (CVE-2026-15925, improper TLS hostname verification; 4.5.0 → 4.7.3), and `gitpython>=3.1.59` (CVE-2026-78675/76/77/78; 3.1.58 → 3.1.62). All three are optional extras (`transformers`/`snowflake`/`git`) and the lockfile already resolved the same major versions, so the floors codify what shipped and block a downstream from forcing an old vulnerable pin. Post-bump OSV audit is clean except the two no-fix advisories already documented and risk-accepted (`chromadb` embedded-only server advisories, `couchbase` CVE-2023-45875). `uv.lock` regenerated.
- **Validated Cassandra keyspace/table identifiers** (#1026, defense-in-depth) — `CassandraVectorStore` interpolates `keyspace`/`table_name` into CQL (identifiers can't be bound as parameters) without validation, unlike every sibling SQL-backed store. It now rejects anything outside `[A-Za-z_][A-Za-z0-9_]*` at construction, closing a latent CQL-injection sink should those names ever become config-/user-derived. The values are developer-controlled today, so valid deployments are unaffected.
- **Authenticated the CAG KV-cache and tightened its storage** (#1024) — the persisted `.bin` is a raw llama.cpp state buffer fed straight into a native C++ deserializer (`LlamaState`/`load_state`), and `KVCacheStore.load` previously validated only integrity metadata (fingerprint/model/n_ctx) which an attacker who can write the cache dir also controls (they write the `.json` too). A crafted blob could therefore reach the native parser (memory-corruption vector). Every blob is now authenticated with HMAC-SHA256 keyed by a per-install secret (auto-generated `0o600` key file, or supply your own via `KVCacheStore(hmac_key=...)`); a missing/invalid HMAC is treated as a **cache miss**, so a tampered or foreign blob never reaches `load_state` — the router just rebuilds or falls back to RAG. The cache directory is now created `0o700` (and tightened if pre-existing) since it holds derived corpus material, and `CacheKey` gains an optional `namespace` for multi-tenant isolation. `CACHE_FORMAT_VERSION` bumped to 2, so pre-existing unsigned caches are transparently invalidated and rebuilt. All additive/non-breaking.
- **Routed the new #889 data loaders through the shared SSRF guard** (#1029) — the loader HTTP helper created an `httpx.Client(follow_redirects=True)` with no URL validation, so a config-derived endpoint (or a `302`/`307` redirect from an otherwise-trusted endpoint) could reach `169.254.169.254` cloud metadata, `127.0.0.1`, or an internal `10.0.0.0/8` service — the same fail-open pattern closed for the agent HTTP tools in #1022/#1027. `http_client` now yields a guard wrapper that disables auto-redirects and re-validates **every hop** via `assert_response_url_public`; the library-owned client also validates the initial URL (an injected client keeps its caller-owned request policy but still has its server-supplied redirect hops validated). Hermetic SSRF regression tests added (private/metadata/loopback/IPv6/`file://` endpoints and redirect-to-private, using IP literals — no network). Also hardened `BoxLoader` so a Box entry field named `source`/`row`/`file_id` can't overwrite the loader's provenance metadata; Box binary-file decoding is tracked as a follow-up (#1030).
- **Added optional authentication to `A2AServer`** (#1023) — the `POST /a2a` endpoint drives `executor.run()` with no auth, so binding it on a network interface let any reachable host drive the agent. `A2AServer` now accepts an `auth_token`; when set, requests must carry a matching `Authorization: Bearer <token>` header (constant-time compared via `hmac.compare_digest`) or receive `401`. When the server binds a non-loopback host (`0.0.0.0`/public) without a token it emits a security `warning`. Non-breaking: `auth_token` defaults to `None` and the default `host` is unchanged; a real loopback HTTP integration test exercises the 401/200 path over TCP.
...
### Added
- **SynapseKit Live now shows the self-evolving loop** (#896 + #732) — `SelfImprovingAgent.evolve()` and `.rollback()` now publish to the Live `EventBus`, so each evolution cycle streams to the dashboard as an `agent.evolve` event (patch status, adopted directive, held-out eval score, or a decoy's block reason) and every rollback as an `agent.rollback`. The dashboard renders both with dedicated icons and story lines. Closes the glass-box gap where the headline self-improvement feature was invisible.
- **Self-evolving agent example notebook** (#732) — `examples/self_evolving_agent.ipynb` is a fully offline, deterministic walkthrough of the governed self-improvement loop: a scripted model, a held-out `EvalSuite` gate, three `observe → propose → validate → canary` cycles that lift held-out accuracy 40% → 100%, decoys blocked by the gate, and a live `agent.rollback(patch_id)`. A lightweight in-process test (`tests/examples/test_self_evolving_notebook.py`, `nbformat` only — no Jupyter kernel) executes every cell and asserts the outcomes so the notebook can't rot. Together with the existing `docs/agents/self-improving.md` and the already-100%-branch-covered `RolloutPolicy`, this closes the last open acceptance criteria on #732.
...
- **`test_z3_unsat_returns_unverified_trace` no longer hard-fails on a broken z3 native library** — `pytest.importorskip("z3")` only catches `ImportError`, but a z3 wheel whose `libz3` can't load (e.g. an arch-mismatched install) raises `z3types.Z3Exception`, so the test errored instead of skipping. It now skips with an explicit reason when z3's native library is unavailable, and still runs for real wherever z3 loads (CI). Surfaced while un-skipping the symbolic suite under `--extra symbolic` for release validation.
